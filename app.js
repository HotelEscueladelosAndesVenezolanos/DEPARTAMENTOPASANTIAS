...
const ESTADOS_PROCESO = [
  "Registrado", "En revisión", "Aprobado", "En pasantías",
  "Culminado", "Rechazado", "Con observación"
];
...
const CONTROL_DOCENTE_FIELDS = [
  "estadoProceso", "tutorAcademico", "empresaAprobada", "fechaRevision",
  "revisadoPor", "observacionesDocentes", "horasRequeridas",
  "horasCumplidas", "resultadoFinal", "observacionesEventualidades"
];

const GESTION_DATOS_PERSONALES_FIELDS = [
  "nombres", "cedula", "fechaNacimiento", "edad", "nacionalidad", "sexo",
  "estadoCivil", "direccionEstudiante", "telefono", "telefonoCantv",
  "correo", "correoInstitucional", "instagram"
];

const GESTION_PASANTIA_EMPRESA_FIELDS = [
  "carrera", "trayecto", "tipo", "tieneUnidadEmpresa", "estado", "ciudad",
  "empresa", "tutorEmpresarial", "telefonoEmpresa", "correoEmpresa",
...
const CHART_COLORS = [
  "#991922", "#c99e2d", "#bb852f", "#6f1118", "#d8b84a",
  "#8a4b22", "#b63b45", "#e7c66a", "#5f5f5f", "#3d3d3d"
];

const FIELD_LABELS = {
  fotoUrl: "Foto", nombres: "Apellidos y nombres completos", cedula: "Cédula de identidad",
  fechaNacimiento: "Fecha de nacimiento", edad: "Edad", nacionalidad: "Nacionalidad",
  sexo: "Sexo", estadoCivil: "Estado civil", direccionEstudiante: "Dirección del estudiante",
  telefono: "Teléfono móvil", telefonoCantv: "Teléfono CANTV", correo: "E-mail personal",
  correoInstitucional: "E-mail institucional", instagram: "Instagram",
  contactoNombre: "Nombre del contacto", contactoTelefono: "Teléfono del contacto",
  contactoCorreo: "E-mail del contacto", contactoParentesco: "Parentesco con usted",
  carrera: "Programa", trayecto: "Semestre / trayecto", otrosEstudios: "Otros estudios realizados",
  estudiosInstitucion: "Institución de estudios", estudiosSemestreEgreso: "Semestre cursante o fecha de egreso",
  estudiosTitulo: "Título alcanzado / por alcanzar", cursosRealizados: "Cursos realizados",
  cursosInstitucion: "Institución del curso", cursosFechaEgreso: "Fecha de egreso del curso",
  cursosCertificado: "Certificado otorgado", cursosCertificadoDetalle: "Detalle del certificado",
  tipoSangre: "Tipo de sangre", alergico: "¿Es alérgico?", alergiaDetalle: "Detalle de alergia",
  enfermedad: "¿Padece alguna enfermedad?", enfermedadDetalle: "Detalle de enfermedad",
  seguroVida: "¿Posee seguro de vida?", hcm: "¿Posee HCM?",
  aseguradoraPoliza: "Empresa aseguradora y número de póliza", telefonoSeguro: "Teléfono / celular del seguro",
  correoSeguro: "Correo electrónico del seguro", madreNombres: "Apellidos y nombres de la madre",
  madreTelefonoMovil: "Teléfono móvil de la madre", madreTelefonoCantv: "Teléfono CANTV de la madre",
  madreCorreo: "E-mail de la madre", madreDireccion: "Dirección de la madre",
  padreNombres: "Apellidos y nombres del padre", padreTelefonoMovil: "Teléfono móvil del padre",
  padreTelefonoCantv: "Teléfono CANTV del padre", padreCorreo: "E-mail del padre",
  padreDireccion: "Dirección del padre", inglesLee: "Inglés - Lee", inglesEscribe: "Inglés - Escribe",
  inglesHabla: "Inglés - Habla", francesLee: "Francés - Lee", francesEscribe: "Francés - Escribe",
  francesHabla: "Francés - Habla", otroIdioma: "Otro idioma", otroIdiomaLee: "Otro idioma - Lee",
  otroIdiomaEscribe: "Otro idioma - Escribe", otroIdiomaHabla: "Otro idioma - Habla",
  tipo: "Tipo de pasantía", tieneUnidadEmpresa: "¿Ya tiene unidad o empresa?", estado: "Estado",
  ciudad: "Ciudad", empresa: "Unidad o empresa de pasantías", tutorEmpresarial: "Tutor empresarial",
  telefonoEmpresa: "Teléfono del tutor empresarial", correoEmpresa: "Correo empresa",
  fechaInicio: "Fecha de inicio", fechaFin: "Fecha de culminación", fechaRegistro: "Fecha de registro",
  estadoProceso: "Estado del proceso", tutorAcademico: "Tutor académico", empresaAprobada: "Empresa aprobada",
  fechaRevision: "Fecha de revisión", revisadoPor: "Revisado por", observacionesDocentes: "Observaciones docentes",
  horasRequeridas: "Horas requeridas", horasCumplidas: "Horas cumplidas", resultadoFinal: "Resultado final",
...
const DEMO_ESTUDIANTES = [
  {
    id: "demo-1", fechaRegistro: "2025-01-10T10:00:00", fotoUrl: "", nombres: "María González",
    cedula: "V-12.345.678", fechaNacimiento: "2004-04-12", edad: "21", nacionalidad: "Venezolana",
    sexo: "Femenino", estadoCivil: "Soltero/a", direccionEstudiante: "Mérida, Venezuela",
    telefono: "0414-0000000", telefonoCantv: "0274-0000000", correo: "maria.demo@email.com",
    correoInstitucional: "maria@unatur.edu.ve", instagram: "@mariademo", contactoNombre: "Laura Pérez",
    contactoTelefono: "0412-1111111", contactoCorreo: "laura@email.com", contactoParentesco: "Tía",
    carrera: "Hotelería", trayecto: "3er Semestre - Hotelería", tipoSangre: "O+", alergico: "NO",
    enfermedad: "NO", seguroVida: "NO", hcm: "NO", madreNombres: "Carmen González",
    madreTelefonoMovil: "0414-2222222", madreTelefonoCantv: "0274-2222222", madreCorreo: "carmen@email.com",
    madreDireccion: "Mérida", padreNombres: "José González", padreTelefonoMovil: "0414-3333333",
    padreTelefonoCantv: "0274-3333333", padreCorreo: "jose@email.com", padreDireccion: "Mérida",
    inglesLee: "SI", inglesEscribe: "SI", inglesHabla: "SI", tipo: "Administrativa",
    tieneUnidadEmpresa: "SI", estado: "Mérida", ciudad: "Mérida", empresa: "Hotel Mérida Plaza",
    tutorEmpresarial: "Laura Pérez", telefonoEmpresa: "0274-0000000", correoEmpresa: "contacto@hotelmerida.com",
    fechaInicio: "2025-02-01", fechaFin: "2025-04-30", estadoProceso: "Registrado",
    empresaAprobada: "En revisión", horasRequeridas: "320", horasCumplidas: "0", resultadoFinal: "Pendiente"
  }
];
...
// ==========================================
// BORRADOR AUTOMÁTICO Y RECUPERACIÓN
// ==========================================
function guardarBorrador() {
  ...
  draft.fotoFileName = $("#fotoFileName")?.value || "";
...
window.addEventListener("beforeunload", (event) => {
  if (tieneBorrador()) {
    ...
    event.returnValue = "Tienes datos sin enviar. ¿Seguro que deseas salir?";
...
function poblarOpcionesFormulario() {
  poblarProgramas(); poblarTiposSangre(); poblarTiposPasantia(); poblarEstados(); poblarEstadoProcesoFiltro();
...
function configurarProgramaTrayecto() {
  const carreraSelect = $("#carrera");
  if (carreraSelect) carreraSelect.addEventListener("change", () => poblarTrayectosPorPrograma(carreraSelect.value));
  const editCarreraSelect = $("#editCarrera");
  if (editCarreraSelect) editCarreraSelect.addEventListener("change", () => poblarTrayectosGestionPorPrograma(editCarreraSelect.value));
}

function poblarTrayectosPorPrograma(programa) {
  const trayectoSelect = $("#trayecto");
  if (!trayectoSelect) return;
  limpiarSelect(trayectoSelect, programa ? "Seleccione semestre / trayecto" : "Seleccione primero un programa");
  if (!programa) return;
  opcionesSistema.trayectos.filter((opcion) => opcion.programa === programa).forEach((opcion) => agregarOption(trayectoSelect, opcion.trayecto, opcion.trayecto));
}
...
function poblarTrayectosGestionPorPrograma(programa, selectedValue = "") {
  ...
  if (!programa) return;
  opcionesSistema.trayectos.filter((opcion) => opcion.programa === programa).forEach((opcion) => agregarOption(trayectoSelect, opcion.trayecto, opcion.trayecto));
  if (selectedValue) trayectoSelect.value = selectedValue;
}
...
function configurarTabs() {
  $$(".tab-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (tab === "dashboard" && !dashboardUnlocked) { abrirLoginDocente(); return; }
      activarTab(tab);
    });
  });
}
...
function configurarFormulario() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      mostrarToast("Error al preparar: " + errorMsg);
    }
  });
...
function configurarConfirmacion() {
  ...
  $("#confirmModal")?.addEventListener("click", (event) => { if (event.target.id === "confirmModal") cerrarConfirmacion(); });
}

function abrirConfirmacion(estudiante) {
  const tieneEmpresa = estudiante.tieneUnidadEmpresa === "SI";
  const camposEmpresa = tieneEmpresa ? ["estado", "ciudad", "empresa", "tutorEmpresarial", "telefonoEmpresa", "correoEmpresa"] : [];
  ...
  function crearSeccionConfirmacion(titulo, campos) {
    return `<section class="confirm-section"><h3>${escapeHTML(titulo)}</h3><div class="confirm-grid">${campos.map((campo) => itemConfirmacion(FIELD_LABELS[campo], valorMostrar(estudiante[campo], campo))).join("")}</div></section>`;
  }
}

function cerrarConfirmacion() { $("#confirmModal").classList.add("hidden"); }
...
// ==========================================
// ENVÍO ROBUSTO CON REINTENTOS Y VALIDACIÓN DE CAMPOS
// ==========================================
...
function configurarDetalleSiNo(selectId, groupId, inputIds) {
  ...
  if (!select.dataset.listenerDetalle) { select.addEventListener("change", actualizar); select.dataset.listenerDetalle = "true"; }
  actualizar();
}
...
function configurarUnidadEmpresa() {
  ...
  if (!select.dataset.listenerUnidad) { select.addEventListener("change", actualizar); select.dataset.listenerUnidad = "true"; }
  actualizar();
}
...
function configurarFiltros() {
  $("#filterEstado")?.addEventListener("change", (event) => { estadoSeleccionado = event.target.value; renderizarTodo(); });
  ...
  $("#studentModal")?.addEventListener("click", (event) => { if (event.target.id === "studentModal") cerrarModal(); });
}
...
function configurarLoginDocente() {
  ...
  $("#loginModal")?.addEventListener("click", (event) => { if (event.target.id === "loginModal") cerrarLoginDocente(); });
  $("#docentePassword")?.addEventListener("keydown", (event) => { if (event.key === "Enter") validarPasswordDocente(); });
...
function cerrarLoginDocente() { $("#loginModal").classList.add("hidden"); }
...
async function validarPasswordDocente() {
  ...
  try {
    const respuesta = await cargarJSONP(CONFIG.APPS_SCRIPT_URL, { action: "login", password });
  ...
  } catch (error) { console.error(error); mostrarToast("No se pudo validar la contraseña."); }
}
...
async function cargarEstudiantes() {
  try {
    if (apiConfigurada()) {
      ...
      if (!passwordDocente) { estudiantes = []; renderizarTodo(); return; }
      mostrarToast("Cargando datos desde Google Sheets...");
      const respuesta = await cargarJSONP(CONFIG.APPS_SCRIPT_URL, { action: "list", password: passwordDocente });
      if (!respuesta.ok) throw new Error(respuesta.message || "No se pudieron cargar los datos.");
...
function cargarJSONP(url, params = {}) {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = () => { delete window[callbackName]; script.remove(); reject(new Error("Error al cargar JSONP")); };
...
function obtenerDatosFormulario(form) {
  ...
  const datos = {};
  for (const [key, value] of formData.entries()) { datos[key] = typeof value === "string" ? value.trim() : value; }
  return datos;
}

function obtenerEstudiantesLocales() {
  try { const data = localStorage.getItem(STORAGE_KEY); return data ? JSON.parse(data).map(normalizarEstudiante) : []; } catch { return []; }
}

function guardarEstudiantesLocales(data) { localStorage.setItem(STORAGE_KEY, JSON.stringify(data)); }

function normalizarEstudiante(item) {
  const base = { id: item.id || generarId(), fechaRegistro: item.fechaRegistro || "" };
  ALL_FIELDS.forEach((field) => { base[field] = item[field] || ""; });
...
function renderizarTodo() { renderizarEstadisticas(); renderizarEstados(); renderizarLista(); renderizarGraficos(); }
...
function renderizarGraficos() {
  ...
  renderizarGraficoCircular("chartEstadosPie", "chartEstadosLegend", contarPorCampo(filtrados, "estado"), "Sin estado");
  renderizarGraficoCircular("chartProgramasPie", "chartProgramasLegend", contarPorCampo(filtrados, "carrera"), "Sin programa");
}

function contarPorCampo(lista, campo) {
  return lista.reduce((acc, item) => { const key = item[campo] || `Sin ${campo}`; acc[key] = (acc[key] || 0) + 1; return acc; }, {});
}
...
function renderizarGraficoCircular(chartId, legendId, conteos) {
  ...
  const entradas = Object.entries(conteos).filter(([, total]) => total > 0).sort((a, b) => b[1] - a[1]);
  const total = entradas.reduce((sum, [, value]) => sum + value, 0);
  if (!total) { chart.style.background = "#f4f4f4"; chart.innerHTML = `<span>0</span>`; legend.innerHTML = `<div class="empty-chart">No hay datos para graficar.</div>`; return; }
  let acumulado = 0;
  ...
  legend.innerHTML = entradas.map(([label, value], index) => {
    const color = CHART_COLORS[index % CHART_COLORS.length];
    const porcentaje = Math.round((value / total) * 100);
    return `<div class="chart-legend-item"><i style="background:${color}"></i><span>${escapeHTML(label)}</span><strong>${value} (${porcentaje}%)</strong></div>`;
  }).join("");
}
...
function renderizarEstados() {
  ...
  opcionesSistema.estados.forEach((estado) => {
    ...
    card.innerHTML = `<strong>${escapeHTML(estado)}</strong><span>${total} estudiante${total === 1 ? "" : "s"}</span>`;
    card.addEventListener("click", () => { estadoSeleccionado = estado; $("#filterEstado").value = estado; renderizarTodo(); });
    contenedor.appendChild(card);
  });
}

function contarPorEstado() { return estudiantes.reduce((acc, estudiante) => { if (!estudiante.estado) return acc; acc[estudiante.estado] = (acc[estudiante.estado] || 0) + 1; return acc; }, {}); }
function calcularNivelCalor(total, maximo) { if (total === 0) return 0; const porcentaje = total / maximo; if (porcentaje <= 0.25) return 1; if (porcentaje <= 0.5) return 2; if (porcentaje <= 0.75) return 3; return 4; }
...
function renderizarLista() {
  ...
  if (filtrados.length === 0) { lista.innerHTML = `<div class="empty-state">No hay estudiantes que coincidan con los filtros seleccionados.</div>`; return; }
...
function abrirFicha(id) {
  ...
  const camposEmpresa = estudiante.tieneUnidadEmpresa === "SI" ? ["estado", "ciudad", "empresa", "tutorEmpresarial", "telefonoEmpresa", "correoEmpresa"] : [];
  $("#modalContent").innerHTML = `
    <div class="profile-head">
      ...
      <div>
        ...
        <p><strong>Programa:</strong> ${escapeHTML(estudiante.carrera || "No registrado")}</p>
        <p><strong>Empresa:</strong> ${escapeHTML(estudiante.empresa || "No registrada")}</p>
        <div class="profile-section-tools"><button type="button" class="btn-primary btn-small" onclick="abrirGestionDocente('${escapeHTML(estudiante.id)}')">Editar control docente</button></div>
      </div>
    </div>
  ...
  function crearSeccionFicha(titulo, campos) { return `<section class="ficha-seccion"><h3>${escapeHTML(titulo)}</h3><div class="profile-grid">${campos.map((campo) => itemFicha(FIELD_LABELS[campo], valorMostrar(estudiante[campo], campo))).join("")}</div></section>`; }
}
...
function crearSeccionIdiomas(estudiante) {
  ...
  return `<section class="ficha-seccion"><h3>Idiomas</h3><div class="language-summary-grid">${filas.map((fila) => `<article class="language-summary-card"><strong>${escapeHTML(fila.idioma)}</strong><div><span>Lee:</span> ${escapeHTML(fila.lee || "NO")}</div><div><span>Escribe:</span> ${escapeHTML(fila.escribe || "NO")}</div><div><span>Habla:</span> ${escapeHTML(fila.habla || "NO")}</div></article>`).join("")}</div></section>`;
}

function cerrarModal() { $("#studentModal").classList.add("hidden"); }
function itemFicha(label, value) { return `<div class="profile-item"><span>${escapeHTML(label)}</span><strong>${escapeHTML(value || "No registrado")}</strong></div>`; }
function valorMostrar(value, campo = "") {
  if (!value) return "No registrado";
  if (["fechaNacimiento", "fechaInicio", "fechaFin", "fechaRegistro", "fechaRevision"].includes(campo)) return formatearFecha(value);
  return value;
}
...
function configurarLimpiezaProceso() {
  ...
  $("#limpiarProcesoModal")?.addEventListener("click", (event) => { if (event.target.id === "limpiarProcesoModal") cerrarLimpiarProceso(); });
  $("#confirmarLimpiarProceso")?.addEventListener("keydown", (event) => { if (event.key === "Enter") limpiarProcesoCompleto(); });
}

function abrirLimpiarProceso() {
  if (!dashboardUnlocked) { abrirLoginDocente(); return; }
...
function cerrarLimpiarProceso() { $("#limpiarProcesoModal")?.classList.add("hidden"); }

async function limpiarProcesoCompleto() {
  const confirmacion = ($("#confirmarLimpiarProceso")?.value || "").trim().toUpperCase();
  if (confirmacion !== "LIMPIAR") { $("#limpiarProcesoError")?.classList.remove("hidden"); return; }
  try {
    if (apiConfigurada()) {
      const password = sessionStorage.getItem(DOCENTE_PASSWORD_KEY) || "";
      if (!password) { mostrarToast("Debes iniciar sesión como docente antes de limpiar el proceso."); return; }
      mostrarToast("Limpiando proceso en Google Sheets...");
      const respuesta = await cargarJSONP(CONFIG.APPS_SCRIPT_URL, { action: "limpiarProceso", password, confirmacion: "LIMPIAR" });
      if (!respuesta.ok) throw new Error(respuesta.message || "No se pudo limpiar el proceso.");
    }
    estudiantes = [];
  ...
  } catch (error) { console.error(error); mostrarToast("No se pudo limpiar el proceso. Verifica Apps Script o la contraseña."); }
}
...
function configurarGestionDocente() {
  ...
  $("#btnCancelarGestion")?.addEventListener("click", () => { if (estudianteGestionSeleccionadoId) cargarFormularioGestion(estudianteGestionSeleccionadoId); });
  ...
  $("#gestionModal")?.addEventListener("click", (event) => { if (event.target.id === "gestionModal") cerrarGestionDocente(); });
}
...
function abrirGestionDocente(id = "") {
  ...
  renderizarGestionLista();
  if (id) seleccionarEstudianteGestion(id);
  else if (estudianteGestionSeleccionadoId) seleccionarEstudianteGestion(estudianteGestionSeleccionadoId);
}

function cerrarGestionDocente() { $("#gestionModal")?.classList.add("hidden"); }
...
function renderizarGestionLista() {
  ...
  lista.innerHTML = "";
  if (filtrados.length === 0) { lista.innerHTML = `<div class="empty-state">No hay estudiantes disponibles.</div>`; return; }
  filtrados.forEach((estudiante) => {
    const button = document.createElement("button");
    button.type = "button";
    button.className = `gestion-student-item ${estudiante.id === estudianteGestionSeleccionadoId ? "active" : ""}`;
    button.innerHTML = `<strong>${escapeHTML(estudiante.nombres || "Sin nombre")}</strong><span>${escapeHTML(estudiante.cedula || "Sin cédula")}</span><small>${escapeHTML(estudiante.estadoProceso || "Registrado")}</small>`;
    button.addEventListener("click", () => seleccionarEstudianteGestion(estudiante.id));
    lista.appendChild(button);
  });
}

function seleccionarEstudianteGestion(id) { estudianteGestionSeleccionadoId = id; renderizarGestionLista(); cargarFormularioGestion(id); }
...
function cargarFormularioGestion(id) {
  ...
  $("#gestionEstudianteResumen").innerHTML = `<img src="${obtenerFoto(estudiante)}" alt="Foto de ${escapeHTML(estudiante.nombres)}" /><div><h3>${escapeHTML(estudiante.nombres || "Sin nombre")}</h3><p><strong>Cédula:</strong> ${escapeHTML(estudiante.cedula || "No registrada")}</p><p><strong>Programa:</strong> ${escapeHTML(estudiante.carrera || "No registrado")}</p><p><strong>Semestre / trayecto:</strong> ${escapeHTML(estudiante.trayecto || "No registrado")}</p><p><strong>Empresa:</strong> ${escapeHTML(estudiante.empresa || "No registrada")}</p></div>`;
  GESTION_DATOS_PERSONALES_FIELDS.forEach((field) => { setGestionValue(campoGestionId(field), valorParaInputGestion(estudiante, field)); });
  setGestionValue("editCarrera", estudiante.carrera);
  poblarTrayectosGestionPorPrograma(estudiante.carrera || "", estudiante.trayecto || "");
  GESTION_PASANTIA_EMPRESA_FIELDS.filter((field) => field !== "carrera" && field !== "trayecto").forEach((field) => { setGestionValue(campoGestionId(field), valorParaInputGestion(estudiante, field)); });
...
function setGestionValue(id, value) { const input = document.getElementById(id); if (input) input.value = value || ""; }

async function guardarGestionDocente(event) {
  event.preventDefault();
  const id = $("#gestionEstudianteId")?.value || "";
  const estudiante = estudiantes.find((item) => item.id === id);
  if (!id || !estudiante) { mostrarToast("Selecciona un estudiante antes de guardar."); return; }
  const data = obtenerDatosGestion();
  try {
    if (apiConfigurada()) {
      const password = sessionStorage.getItem(DOCENTE_PASSWORD_KEY) || "";
      const respuesta = await cargarJSONP(CONFIG.APPS_SCRIPT_URL, { action: "updateDocente", password, id, cedulaOriginal: estudiante.cedula || "", ...data });
      if (!respuesta.ok) throw new Error(respuesta.message || "No se pudo guardar el control docente.");
    }
    ...
    if ($("#studentModal") && !$("#studentModal").classList.contains("hidden")) abrirFicha(id);
    mostrarToast("Cambios guardados correctamente.");
  } catch (error) { console.error(error); mostrarToast(error.message || "No se pudieron guardar los cambios."); }
}
...
function agregarObservacionGestion() {
  ...
  if (!texto) { mostrarToast("Escribe la observación antes de agregarla."); textoInput.focus(); return; }
  const fechaMostrar = formatearFecha(fecha);
  const nuevaLinea = `${fechaMostrar} - ${texto}`;
  historialInput.value = historialInput.value.trim() ? `${historialInput.value.trim()}\n${nuevaLinea}` : nuevaLinea;
  fechaInput.value = "";
  textoInput.value = "";
...
function formatearFecha(valor) {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(valor)) { const [year, month, day] = valor.split("-"); return `${day}/${month}/${year}`; }
...
function normalizarFechaInput(valor) {
  ...
  if (/^\d{2}\/\d{2}\/\d{4}$/.test(valor)) { const [day, month, year] = valor.split("/"); return `${year}-${month}-${day}`; }
  const fecha = new Date(valor);
