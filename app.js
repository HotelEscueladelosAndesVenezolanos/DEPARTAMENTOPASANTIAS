...
const CONTROL_DOCENTE_FIELDS = ["estadoProceso", "tutorAcademico", "empresaAprobada", "fechaRevision", "revisadoPor", "observacionesDocentes", "horasRequeridas", "horasCumplidas", "resultadoFinal", "observacionesEventualidades"];
...
const FIELD_LABELS = {
  fotoUrl: "Foto", nombres: "Apellidos y nombres completos", cedula: "Cédula de identidad", fechaNacimiento: "Fecha de nacimiento", edad: "Edad", nacionalidad: "Nacionalidad", sexo: "Sexo", estadoCivil: "Estado civil", direccionEstudiante: "Dirección del estudiante", telefono: "Teléfono móvil", telefonoCantv: "Teléfono CANTV", correo: "E-mail personal", correoInstitucional: "E-mail institucional", instagram: "Instagram", contactoNombre: "Nombre del contacto", contactoTelefono: "Teléfono del contacto", contactoCorreo: "E-mail del contacto", contactoParentesco: "Parentesco con usted", carrera: "Programa", trayecto: "Semestre / trayecto", otrosEstudios: "Otros estudios realizados", estudiosInstitucion: "Institución de estudios", estudiosSemestreEgreso: "Semestre cursante o fecha de egreso", estudiosTitulo: "Título alcanzado / por alcanzar", cursosRealizados: "Cursos realizados", cursosInstitucion: "Institución del curso", cursosFechaEgreso: "Fecha de egreso del curso", cursosCertificado: "Certificado otorgado", cursosCertificadoDetalle: "Detalle del certificado", tipoSangre: "Tipo de sangre", alergico: "¿Es alérgico?", alergiaDetalle: "Detalle de alergia", enfermedad: "¿Padece alguna enfermedad?", enfermedadDetalle: "Detalle de enfermedad", seguroVida: "¿Posee seguro de vida?", hcm: "¿Posee HCM?", aseguradoraPoliza: "Empresa aseguradora y número de póliza", telefonoSeguro: "Teléfono / celular del seguro", correoSeguro: "Correo electrónico del seguro", madreNombres: "Apellidos y nombres de la madre", madreTelefonoMovil: "Teléfono móvil de la madre", madreTelefonoCantv: "Teléfono CANTV de la madre", madreCorreo: "E-mail de la madre", madreDireccion: "Dirección de la madre", padreNombres: "Apellidos y nombres del padre", padreTelefonoMovil: "Teléfono móvil del padre", padreTelefonoCantv: "Teléfono CANTV del padre", padreCorreo: "E-mail del padre", padreDireccion: "Dirección del padre", inglesLee: "Inglés - Lee", inglesEscribe: "Inglés - Escribe", inglesHabla: "Inglés - Habla", francesLee: "Francés - Lee", francesEscribe: "Francés - Escribe", francesHabla: "Francés - Habla", otroIdioma: "Otro idioma", otroIdiomaLee: "Otro idioma - Lee", otroIdiomaEscribe: "Otro idioma - Escribe", otroIdiomaHabla: "Otro idioma - Habla", tipo: "Tipo de pasantía", tieneUnidadEmpresa: "¿Ya tiene unidad o empresa?", estado: "Estado", ciudad: "Ciudad", empresa: "Unidad o empresa de pasantías", tutorEmpresarial: "Tutor empresarial", telefonoEmpresa: "Teléfono del tutor empresarial", correoEmpresa: "Correo empresa", fechaInicio: "Fecha de inicio", fechaFin: "Fecha de culminación", fechaRegistro: "Fecha de registro", estadoProceso: "Estado del proceso", tutorAcademico: "Tutor académico", empresaAprobada: "Empresa aprobada", fechaRevision: "Fecha de revisión", revisadoPor: "Revisado por", observacionesDocentes: "Observaciones docentes", horasRequeridas: "Horas requeridas", horasCumplidas: "Horas cumplidas", resultadoFinal: "Resultado final", observacionesEventualidades: "Observaciones / eventualidades"
};
...
function configurarFormulario() {
  ...
  form.addEventListener("reset", () => {
    setTimeout(() => {
      ...
      configurarCamposCondicionales();
      limpiarBorrador(); // 5. LIMPIEZA DE RESPALDO al resetear
    }, 0);
  });
...
function enviarRegistroConfirmado() {
  ...
  if (apiConfigurada()) {
    ...
    (async () => {
      try {
        // 6. MANEJO DE CAMPOS VACÍOS: Asegurar que todos los campos sean strings válidos para evitar corrimiento de celdas
        const datosEnvio = {};
        ...
        datosEnvio.action = 'register';
        datosEnvio.id = registroPendiente.id || generarId();
        datosEnvio.fechaRegistro = registroPendiente.fechaRegistro || new Date().toISOString();
        datosEnvio.fotoUrl = registroPendiente.fotoUrl || "";
        datosEnvio.fotoBase64 = registroPendiente.fotoUrl || ""; // El backend espera este nombre exacto
        datosEnvio.fotoFileName = $("#fotoFileName")?.value || "";
        datosEnvio.timestamp = new Date().toISOString();
        ...
        mostrarToast("Ocurrió un error inesperado al enviar el registro.");
      } finally {
        // REACTIVAR BOTÓN (siempre, para permitir nuevo intento si falló)
        if (btnEnviar) {
